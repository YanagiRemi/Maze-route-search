...
OL=deque([(start_line,start_row)])  #スタート位置の座標が入ったオープンリスト作成　構造はキュー
CL=[]                               #空のクローズドリスト作成　

while True:                         #while文開始
    if (OL == deque([])):  # もしオープンリストの中身が空であればwhile文を抜ける
        break
    st=OL.popleft()                 #OLをデキューして取り出した座標をstに代入
    CL.append(st)                   #クローズドリストにstの座標を追加
    if(st==(gole_line,gole_row)):   #stがゴールの座標であればwhile文を抜ける
        break
    else:
        B = next(st, CL, maze)      #変数Bにnext関数の返り値代入
        for i in B:                 #リストBの中身を順にエンキューしてオープンリストに追加
            OL.append(i)
